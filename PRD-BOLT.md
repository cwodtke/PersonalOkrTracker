...
**Priority: P1 (Should Have)** (Basic version)
...
- Magic link authentication via email (primary method)
  - Users log in with email address
  - System generates secure, single-use magic link
  - Magic links expire after 24 hours
  - Automatic account creation on first login
- Session management (localStorage or JWT tokens)
- No password required (passwordless authentication)

**Email Configuration:**
- Configure email service (Nodemailer + Gmail/SendGrid)
- HTML email templates for magic links
- Daily reminder email system
...
- Database with Write-Ahead Logging (WAL) for concurrency
- Regular automated backups
- Secure session management
- All user data associated with userId for complete isolation
...
- Client-side routing
- CSS for styling (modular approach)
...
- Job Scheduler: node-cron or similar for daily emails

#### Email Service
- Transactional email provider (Nodemailer + Gmail/SendGrid)
- HTML email templates
- Magic link generation and validation
- Daily reminder scheduling

#### Infrastructure
- Cloud hosting (flexible deployment options)
...
**Document Version**: 2.0 (Bolt.new Template)
...
1. **Phase 1 (MVP - Core Features):**
   ...
   - Health Metrics (Red/Yellow/Green status)
   - Heartbeat Work tracking
   - Task Management with status dropdown (Not Started/In Progress/Done)
   - Subtasks with visual hierarchy
   - Responsive design (mobile + desktop)
   - Full-width layout

2. **Phase 2 (Authentication & Email):**
   - Magic Link Authentication
